...
Snakes and Ladders is typically played with 2 players on an $n$ by $n$ grid. Each user takes turns rolling a 6-sided dice, gradually progressing up the board until they reach the final square. Along the way they can land on 'snakes', which takes them down to previous squares, or 'ladders', which takes them up closer to the end. 
We will look at only the one player case with a few key differences:
