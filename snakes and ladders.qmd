...
Will the simulated average converge to our calculated theoretical value? Let us rerun the simulation 100 times, taking the average each time. We can then perform a one-sampled $t$-test where, $H_0:\mu=48.1$ and $H_1:\mu\ne48.1$.
...
t.test(avg, mu=48.1,alternative="two.sided")
```
...
qqnorm(avg) 
qqline(avg)
t.test(mean(run_time), mu=47.98, alternative="two.sided")
mean(run_time)
sd(run_time)
